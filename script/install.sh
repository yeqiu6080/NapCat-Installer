#!/bin/bash
###
 # @Author: yeqiu6080 yeqiu6080@outlook.com
 # @Date: 2025-05-10 16:27:39
 # @LastEditors: yeqiu6080 yeqiu6080@outlook.com
 # @LastEditTime: 2025-05-10 16:54:53
 # @FilePath: \NapCat-Installer\script\install.sh
...
function check_napcat_cli() {
    if [ "${use_cli}" = "y" ]; then
        ...
        log "跳过安装/更新 CLI (用户未选择或使用 --cli n)。"
    fi
}
...
function install_napcat_cli() {
    ...
    log "安装NapCatQQ CLI..."   
    network_test "Github"
    napcat_cli_download_url="${target_proxy:+${target_proxy}/}https://raw.githubusercontent.com/yeqiu6080/NapCat-Installer/refs/heads/main/script/napcat"
...
function show_main_info() {
    ...
    if [ "${use_cli}" = "y" ]; then
        ...
        log "${YELLOW}未安装 CLI 工具。如需使用便捷命令管理, 请重新运行安装脚本并选择安装 CLI (--cli y)。${NC}"
    fi
    log "--------------------------------------------------"
}
# TODO：TUI
